...
# - The reads considered go from right to left
...
for line in sam_file:
	...
	if ((int(fields[1]) & 12) == 0):   # flag unset indicates that both segment map 
		if (int(fields[8])  < 0) and ((int(fields[8])  > - max_fragment_size)):	# consider a negative max_fragment_size					      
			...
			genome_change[end_read_position - mate_length] -= 1 # - because the fragment to read from right to left
sam_file.close()
...
for position in range(genome_length-1, -1, -1): # genome_change[genome_length-1...0]
	...
	reverse.append(current_coverage) # reverse[genome_length-1...0]
for i in range(genome_length-1, -1, -1):
	print(reverse[i]) # print position bases from reverse[genome_length]=0 until to reverse[0]=genome_length-1
